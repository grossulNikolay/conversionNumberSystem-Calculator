public class ConverterToDecimal {
    ...
    public static final int[] factNumbers = new int[]{1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800,
            39916800, 479001600}; //задали 2 неизменяемых массива для Факт и Фиб СС

    public static int convertAtoDecimal(String numberA, String systemB) { //10-ричные можно в int
        ...
        try { //числовые СС (2-36) +нега-позиционные
        ...
        int decimalNumber = Integer.parseInt(numberA, radix); //числовые СС (2-36)
        return decimalNumber;

        } catch (Exception error) {
            return 0; //костыль, как в других методах (без этой строчки) не получается -- выдает ошибку
        }
    }
}
